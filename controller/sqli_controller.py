...
class AutoSQLAttack(Resource):
    def post(self):
        ...
        for k, v in fields.items():
            ...
            test = Test(form_id, 'sql', json.dumps(temp_fields), file_path, img_path, datetime.now(), duration, vulnerable)
            test.save_to_db()

        driver.quit()
        return {'status':'ok'}

class ManualSQLAttack(Resource):
    def post(self):
        parser = reqparse.RequestParser()
        ...
        test = Test(form_id, 'sql', json.dumps(data), file_path, img_path, datetime.now(), duration, vulnerable)
        test.save_to_db()
